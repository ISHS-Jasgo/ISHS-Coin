...
var kokoa;
var nestla;
const deal_category = document.getElementById("deal-category");
const ID = window.localStorage.getItem("ID");
const stocks = ["Sungsam", "PineApple", "Kokoa", "Nestla"];
...
async function order(name) {
  if (deal_category.value == "매수") {
    app.buy(ID, stocks.indexOf(name), Number(document.getElementById("deal-count").value));
    update();
  } else {
    app.sell(ID, stocks.indexOf(name), Number(document.getElementById("deal-count").value));
    update();
  }
}
